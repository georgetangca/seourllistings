...
function generate_slug($str,$divider){
...
function ajax_redirect_listing(){
    get_sidebar_filter();
...
function fuli_load_listing_on_click(){ //for server full 
    get_sidebar_filter();
...
function loadSort(sort_string, sort_id) { //this function is fired when the sort is clicked
...
function set_except_sidebar_status(){
        ...
        success: function (data) {
            ...
            if (data['listings'].length < 1) { //no listings
                ...
                $.each(data['listings'], function (i, item) { //build listing summary for dispaly
                   ...
                   // $stringData += '<input type="hidden" value="'+map_point.toSource()+'" />';
                   // $stringData += '<p class="cuisine">'+ show_cuisine(cuisine)+'</p>';
                   // $stringData += '<p class="neighbor">'+ show_neigbourhood(neighbourhood) +'</p>';
                    
                    /*
                    var rating = "";
                    if(item.userrating != null){
                        rating += Math.floor(item.userrating);
                    }
                    
                    var sortArray = rating.split(".");
                    var star_class_name = "star rating";
                    if(sortArray[0] != ''){
                    ...
                    $stringData += '<p class="'+star_class_name+'">' + '&nbsp;' +'</p>';
                    */
                    //$stringData += '<p class="map"><button class="map_show" value="'+item.id+'">Map</button></p>'
...
function currentListsMap(locations) {
...
function check_canadian_rockie_pillar(){
...
function showMap(locations, index) {
    var map_zoom;
    var latlng;
...
function getListingURL(slug,catslug,catlevel1slug) {
